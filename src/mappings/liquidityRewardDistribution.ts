...
import { UserRewardDistributionMetadata } from '../types/templates';
...
export function handleEpochAdded(event: EpochAdded): void {
  ...
  UserRewardDistributionMetadata.create(ipfsCid);

  snapshot.save();
}

export function handleMetadata(content: Bytes): void {
  log.warning("Handler datasource {}", [dataSource.stringParam()]);
  ...
  for (let index = 0; index < distributionsData.length; index++) {
    ...
    let key = dataSource.stringParam() + "-" + distribution.address;
